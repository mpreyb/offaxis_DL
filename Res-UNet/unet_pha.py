...
'''
    ...
    Outputs: In-focus phase reconstructions.
...
# Batch data loading class with data augmentation
class HoloAmpDataGenerator(tf.keras.utils.Sequence):
    ...
    def __data_generation(self, holo_batch, amp_batch):
        ...
        for i, (holo_file, amp_file) in enumerate(zip(holo_batch, amp_batch)):
            ...
            pha_img = tf.keras.preprocessing.image.load_img(amp_file, color_mode='grayscale', target_size=self.target_size)
            pha_img = tf.keras.preprocessing.image.img_to_array(pha_img) / 255.0

            # Apply data augmentation
            if self.augment:
                holo_img, pha_img = self.apply_augmentation(holo_img, pha_img)
...
(train_holo_files, train_pha_files), (val_holo_files, val_pha_files), (test_holo_files, test_pha_files) = split_data(holo_files, pha_files)

# Create data generators
train_gen = HoloAmpDataGenerator(train_holo_files, train_pha_files, batch_size=batch_size, target_size=target_size)
val_gen = HoloAmpDataGenerator(val_holo_files, val_pha_files, batch_size=batch_size, target_size=target_size)
test_gen = HoloAmpDataGenerator(test_holo_files, test_pha_files, batch_size=batch_size, target_size=target_size)
...
print("- Number of phases for training", len(train_pha_files))
print("- Number of phases for validation", len(val_pha_files))
print("- Number of phases for test", len(test_pha_files))
...
####################################################################################################################################
#%% MAKING SURE EACH HOLOGRAM IS BEING ASSIGNED THE CORRECT RECONSTRUCTION
...
def save_random_three_from_generator(data_generator, save_dir, num_samples=3):
    ...
    for i in sampled_indices:
        ...
        plt.subplot(1, 2, 2)
        plt.title('Phase')
        plt.imshow(np.squeeze(pha_img), cmap='gray')
        plt.axis('off')
...
# Example usage
save_random_three_from_generator(train_gen, 'MATCH_UNET')
