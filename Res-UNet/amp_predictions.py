...
test_samples, test_amp = test_gen.__getitem__(0)  # Get the first batch

# Making predictions
predicted_amp = unet.predict(test_samples)
...
for i in range(len(test_samples)):
    ...
    plt.subplot(1, 3, 2)
    plt.imshow(test_amp[i].reshape(512, 512), cmap='gray')
    plt.title(f'True Amplitude {i+1}')

    # Display predicted amplitude
    plt.subplot(1, 3, 3)
    plt.imshow(predicted_amp[i].reshape(512, 512), cmap='gray')
    plt.title(f'Predicted Amplitude {i+1} (SSIM: {ssim_scores[i]:.3f})')
